 ...
 *
 * Notes:
 * - This script drops the app collections it touches (users, properties, bookings, favorites, messages, blogs).
...
async function seed() {
  try {
    if (!process.env.MONGO_URI) {
      console.error(
        'MONGO_URI not set in environment. Copy .env.example to .env and set MONGO_URI.'
      );
      process.exit(1);
    }
    ...
    for (let i = 0; i < seedData.properties.length; i += 1) {
      ...
      const prop = new Property({
        ...
        owner: owner._id,
        status: 'published'
        // IMPORTANT: do not override images here.
        // If seedData.properties[i] has `images`, they will be saved.
        // images: p.images, // optional but not required if already in `p`
      });
    ...
    const bob =
      createdUsers.find(x => x.email === 'bob@example.com') ||
      createdUsers.find(u => u.role === 'user' && u.email !== 'alice@example.com');
    ...
    if (bob && alice && createdProps[0] && seedData.messages[0]) {
    ...
    if (alice && manager && seedData.messages[1]) {
    ...
    const createdBlogs = [];
    if (Array.isArray(seedData.blogs)) {
      for (const b of seedData.blogs) {
        const blog = new Blog(b); // slug generated by pre('save') hook
        await blog.save();
        createdBlogs.push(blog);
        console.log(`Created blog "${blog.title}" with slug "${blog.slug}"`);
      }
    } else {
      console.log('No blogs array found in seedData; skipping blog seeding.');
    }
    ...
    createdUsers.forEach(u =>
      console.log(`- ${u.email} / password: (check env or default)`)
    );
    process.exit(0);
  } catch (err) {
